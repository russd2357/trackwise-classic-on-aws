...
export class TwcProdStack extends cdk.Stack {
    ...
    constructor(scope: Construct, id: string, props?: cdk.StackProps ) {
        ...
        this.vpc = new ec2.Vpc(this, 'twc-prod-vpc', {
            cidr: '10.2.0.0/16',
            maxAzs: 2,
            natGateways: 1,     
            subnetConfiguration: [
        ...
        this.ssmrole = new Role(this, 'twc-prod-ssmrole', {
    ...
    private CreateSecurityGroups(): void {
        let sgPublic = new ec2.SecurityGroup(this, 'twc-prod-sg-public' , {
            vpc: this.vpc,
        ...
        let sgAppPrivate = new ec2.SecurityGroup(this, 'twc-prod-sg-priv-app', {
            vpc: this.vpc,
        ...
        let sgAppDb = new ec2.SecurityGroup(this, 'twc-prod-sg-priv-db', {
            vpc: this.vpc,
    ...
    private LaunchInstances(): void {
        ...
        let dbserver = new rds.DatabaseInstance(this, 'twc-prod-dbserver', {
            vpc: this.vpc,
        ...
        let qvappserver = new ec2.Instance(this, 'twc-prod-qv-appserver', {
            vpc: this.vpc,
            instanceType: ec2.InstanceType.of(ec2.InstanceClass.M4, ec2.InstanceSize.LARGE),
            ...
            keyName: 'twc-prod-key',
            role: this.ssmrole
        });
    }

    private LaunchFrontend() {
        let alb = new ApplicationLoadBalancer(this, 'twc-prod-alb', {
            vpc: this.vpc,
            vpcSubnets: this.vpc.selectSubnets({ subnetGroupName: 'sn-priv-app'}),
            internetFacing: true,
            loadBalancerName: 'twc-prod-alb',
        })

        let asg = new AutoScalingGroup(this, 'twc-prod-asg', {
            vpc: this.vpc,
            vpcSubnets: this.vpc.selectSubnets({subnetGroupName: 'sn-priv-app'}),
            instanceType: ec2.InstanceType.of(ec2.InstanceClass.M4, ec2.InstanceSize.LARGE),
            machineImage: ec2.MachineImage.latestWindows(ec2.WindowsVersion.WINDOWS_SERVER_2012_R2_RTM_ENGLISH_64BIT_HYPERV),
